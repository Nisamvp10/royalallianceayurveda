...
<?= $this->section('scripts') ?>
 ...
 loadSuppliers();
            function loadSuppliers(search = '') {
                ...
                $.ajax({
                    url: "<?=  base_url('admin/'.slugify(getappdata('clients')).'/list') ?>",
                    type: "POST",
                                ...
                                <div class="px-3 py-3 whitespace-nowrap_">
                                    <div class="flex items-start">
                                        ${client.profileimg ? 
                                            ...
                                            `<img src="${client.profileimg}?auto=compress&amp;cs=tinysrgb&amp;w=800" alt="Deep Tissue Massage" class="h-12 w-12 rounded-full mr-4">`
                                            :  `<div class="h-9 w-9 rounded-full bg-blue-100 flex items-center justify-center mr-3" >
                                            <span class="text-blue-600 text-medium"> ${client.name.charAt(0)}</span></div>
                                            ` 
                                        }
                                            <div class="flex-1">
                                                <div class="flex items-center justify-between">
    ...
    function deleteSupplier(e) {
        if (confirm("are you sure")) {
            let id = $(e).data('id');
             $.ajax({
                url : '<?= base_url('admin/'.slugify(getappdata('clients')).'/delete/');?>'+id,
                method:'POST',
                data: {id:id},
                dataType : 'json',
                success:function(response)
    ...
    function activeSupplier(e) {
        if (confirm("are you sure")) {
            let id = $(e).data('id');
             $.ajax({
                url : '<?= base_url('admin/'.slugify(getappdata('clients')).'/active/');?>'+id,
                method:'POST',
                data: {id:id},
                dataType : 'json',
                success:function(response)
