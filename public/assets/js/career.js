function openModal(id=false) {
    ...
    modal.find('.head').text('Add New Career');
    let webForm = document.getElementById('careerForm');
    webForm.querySelector('#title').value = '';
    webForm.querySelector('#status').value = '';
    webForm.querySelector('#duration').value = '';
    webForm.querySelector('#note').value = '';
    webForm.querySelector('#description').value = '';
    tinymce.get('description').setContent('');
    webForm.querySelector('#category').value = '';
    webForm.querySelector('#vacancy').value = '';
    webForm.querySelector('#jobcode').value = '';
    webForm.querySelector('#salary').value = '';
    webForm.querySelector('#apply_on').value = '';
    webForm.querySelector('#location').value = '';
    webForm.querySelector('#highlights_title').value = '';
    ...
    $('#edit_id').val('');
     modal.find('.head').text('Edit New Career');
    if(id) {
        $('#edit_id').val(id);
       fetch(App.getSiteurl() +'admin/edit-career/'+`${id}`)
       .then(res => res.json())
       .then(result =>{
        if(result) {
                ...
                webForm.querySelector('#category').value = result[0].category;
                webForm.querySelector('#vacancy').value = result[0].vacancy;
                webForm.querySelector('#jobcode').value = result[0].jobcode;
                webForm.querySelector('#salary').value = result[0].salary;
                webForm.querySelector('#apply_on').value = result[0].apply_on;
                webForm.querySelector('#location').value = result[0].location;
                webForm.querySelector('#edit_id').value = result[0].careerId;
                webForm.querySelector('#highlights_title').value= result[0].highlights_title;
                if( result[0].image) {
...
$(document).on('click', '.removeRow', function () {
    ...
    $.ajax({
        url : App.getSiteurl() +'admin/careers/save',
        method:'POST',
        data: formData,
        contentType: false,
        processData: false,
...
function loadCareers(search ='') {
    $.ajax({
        url : App.getSiteurl() +'admin/careers/data',
        method:'POST',
        data:{search:search},
        dataType: 'json',
...
function deleteItem(e){
    ...
    if(confirm('are you sure You want to Delete This')) {
        if(id) {
            $.ajax({
                 url : App.getSiteurl()+'admin/careers/delete/'+ id,
                type: "DELETE",
                dataType: 'json',
                success:function(response)
                { 
