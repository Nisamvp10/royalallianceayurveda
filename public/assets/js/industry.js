function openModal(id=false) {
    toggleModal('industryModal', true);
    let modal = $('#industryModal');
    let mdlTitle = $('.head').data('title');
    modal.find('.head').text('Add ' + mdlTitle);
    const webForm = document.getElementById('industryForm');
    $('#edit_id').val('');
    ...
    if(id) {
        $('#edit_id').val(id)
       fetch(App.getSiteurl() +'admin/edit-industries/'+`${id}`)
       .then(res => res.json())
       .then(result =>{
        if(result) {
            console.log(result);
            $('#industryModal').find('.head').text('Edit ' + mdlTitle);
            ...
            webForm.querySelector('#description').value = result.description;
             if(result.image) {
                ...
                ? result.image 
                : App.getSiteurl() + 'uploads/industries/default.jpg';
            }
        }
       })    
    }
...
function loadData(search = '') {
    ...
    $.ajax({
        url: App.getSiteurl() +'admin/industries/data',
        method : 'POST',
        data:{search:search},
        dataType: 'json',
...
$('#industryForm').on('submit', function(e) {
    ...
    $.ajax({
        url : App.getSiteurl() +'admin/industries/save',
        method:'POST',
        data: formData,
        contentType: false,
        processData: false,
...
function deleteItem(e){
    ...
    if(confirm('are you sure You want to Delete This')) {
        if(id) {
            $.ajax({
                 url : App.getSiteurl()+'admin/industries/delete/'+ id,
                type: "DELETE",
                dataType: 'json',
                success:function(response)
                { 
